...
# Maps Celery's internal states to the application's job state vocabulary.
...
def enqueue_job(task_name: str, *args, **kwargs) -> str:
    ...
    Uses .delay() (not send_task) so that task_always_eager=True in tests
    executes the task synchronously within the same process.
    """
    from tasks import send_notification_job, process_event_job, cleanup_old_notifications_job
    _tasks = {
...
def get_job_status(job_id: str) -> dict:
    ...
    state = _CELERY_TO_JOB_STATE.get(raw_state, "pending")
    return {
        "job_id": job_id,
        "state": state,
        "result": (
